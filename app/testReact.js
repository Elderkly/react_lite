...
//  render Fun
function render($parent, oldNode, newNode, index = 0) {
    //  获取oldNode的真实Dom
    ...
    if (isObjectChanged(oldNode, newNode)) {
        ...
        if (newPropsKeys.length === 0) {
            ...
            allPropsKeys.forEach(prop => {
                ...
                //     $currentNode[event] = newProps[prop] ? newProps[prop] : oldProps[prop]
                // }
    ...
    if ((oldNode.children && oldNode.children.length) || (newNode.children && newNode.children.length)) {
        ...
        for (let i = 0; i < maxLength; i++) {
            render($currentNode, oldNode.children[i], newNode.children[i], i)
        }
    }
}
...
//     <div>
//         <p>第三层</p>
//         <div className={"ending"} onClick={() => {alert(123123)}}>
//             <p>第四层</p>
//         </div>
//     </div>
// </div>
// //  插入元素
// React.render($app,oldDom,newDom)
// //  延时刷新元素
// setTimeout(() => React.render($app,newDom,nodeChange),5000)
